...
test("backstage converges governance into one settings entry", () => {
  ...
  assert.match(source, /const interactionStatus = workbenchStatusFromToolInteraction\(parseToolInteraction\(metadata\)\);[\s\S]*if \(interactionStatus\) return interactionStatus;/);
  ...
  assert.match(source, /JSON\.stringify\(approvalBlockers, null, 2\)/);
