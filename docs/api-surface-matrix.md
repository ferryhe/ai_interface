...
| --- | --- | --- | --- |
| GET | `/api/agent-config` | `admin` | Localhost + same-origin/forwarded-host admin guard with loopback forwarded-client and same-origin Fetch Metadata checks; rejects cross-site and `agent-portal` surface before reading config. Ordinary Mission first load must not call it. |
| PUT | `/api/agent-config` | `admin` | Localhost + same-origin/forwarded-host admin guard with loopback forwarded-client and same-origin Fetch Metadata checks; rejects cross-site and `agent-portal` surface before validating or writing config. |
| POST | `/api/agent-config/test-connection` | `admin` | Localhost + same-origin/forwarded-host diagnostic guard with loopback forwarded-client and same-origin Fetch Metadata checks; must not leak secrets. |
...
| GET | `/api/agents` | `backstage` | Local admin read guard; capability registry read; not ordinary frontstage chrome. |
| GET | `/api/agents/{agentId}/export/vscode-agent` | `backstage` | Local admin read guard via `/api/agents/*`; export metadata only; redacts provider internals and local paths. |
| GET | `/api/agents/{agentId}/export/mcp-tool` | `backstage` | Local admin read guard via `/api/agents/*`; export metadata only; redacts provider internals and local paths. |
...
| GET | `/api/climate-monitor/status` | `internal`, `admin` | Local admin read guard; redacted local workflow status. |
...
| GET | `/api/modules` | `backstage`, `internal` | Local admin read guard; runtime catalog/readiness; not ordinary frontstage chrome. |
...
| GET | `/api/skills` | `backstage` | Local admin read guard; skill registry/readiness; not ordinary frontstage chrome. |
| POST | `/api/skill-manifests` | `admin` | Guarded writer: `AI_INTERFACE_MANIFEST_WRITE_MODE=custom`, localhost, same-origin. |
| GET | `/api/teams` | `backstage` | Local admin read guard; derived team/agent registry read. |
| GET | `/api/tool-adapters` | `backstage` | Local admin read guard; adapter metadata and redacted readiness only. |
